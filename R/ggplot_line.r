...
data <- read.table(dataFN,sep='\t',header=T) # xValue, yValue, group, sd
data <- na.omit(data)
...
p <- ggplot(data,aes(x=xValue,y=yValue,group = group, color = group)) +
	geom_errorbar(aes(ymin = yValue - sd, ymax = yValue + sd), width = .1) +
	# geom_line(aes(linetype = group)) +
	geom_point(aes(shape = group)) +
	geom_line() +
	scale_color_manual(values = c('grey', '#A14C94', '#87C232', '#C7A609', '#16557A', '#CE0013')) +
