...
library(ggplot2)
data <- read.table(dataFN,sep='\t',header=T) # format: xValue, yValue, group
data <- na.omit(data)
...
ggplot(data,aes(x=xValue,y=yValue,fill=group)) +
	geom_violin(position = dodge) +
	geom_boxplot(width = 0.02, outlier.colour = NA, position = dodge) +
	# stat_summary(fun.y = mean, geom = 'point', shape = 23, size = 2, color = 'white', position = dodge) +
